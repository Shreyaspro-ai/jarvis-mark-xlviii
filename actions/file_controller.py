...
def _resolve_path(raw: str) -> Path:
    ...
    # Handle paths that START with a shortcut segment, e.g. "downloads/AgentTest"
    # or "desktop\\notes\\a.txt" — resolve the first segment to the real folder.
    norm = raw.replace("\\", "/")
    first, sep, rest = norm.partition("/")
    if sep and first.lower() in shortcuts and rest:
        return shortcuts[first.lower()] / rest

    p = Path(raw).expanduser()
    # A bare relative path would otherwise resolve against Jarvis's program
    # directory, hiding files where the user can't find them. Anchor to home.
...
def open_path(path: str, name: str = "") -> str:
    ...
    try:
        ...
        if _OS == "Windows":
            os.startfile(str(target))  # noqa: opens in Explorer / default app
        elif _OS == "Darwin":
            import subprocess
